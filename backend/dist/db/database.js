...
function toPostgresParameters(sql) {
...
async function getDb() {
    return {
        async get(sql, params = []) {
            const rows = await prisma.$queryRawUnsafe(toPostgresParameters(sql), ...params);
            return rows[0];
        },
        async all(sql, params = []) {
            return prisma.$queryRawUnsafe(toPostgresParameters(sql), ...params);
        },
        async run(sql, params = []) {
            const changes = await prisma.$executeRawUnsafe(toPostgresParameters(sql), ...params);
            return { changes };
        },
        async exec(sql) {
...
async function initDb() {
  ...
  `);
    ...
    for (const [userId, username] of demoCredentials) {
        await db.run('UPDATE users SET username = ?, password_hash = ? WHERE user_id = ? AND (username IS NULL OR password_hash IS NULL)', [username, crypto.createHash('sha256').update('demo123').digest('hex'), userId]);
    }
    // Seeding
...
async function seedDatabase(db) {
    ...
    for (const u of users) {
        await db.run('INSERT INTO users (user_id, name, email, username, password_hash, mfa_enabled, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)', [u.user_id, u.name, u.email, u.username, crypto.createHash('sha256').update(u.password).digest('hex'), u.mfa_enabled, nowStr]);
        // Link user to role
    ...
    for (const o of orgs) {
        await db.run('INSERT INTO organizations (org_id, name, org_type, created_at) VALUES (?, ?, ?, ?)', [o.org_id, o.name, o.org_type, nowStr]);
    }
    // 4. Studies
    const studies = [
        {
    ...
    for (const s of studies) {
        await db.run('INSERT INTO studies (study_id, org_id, title, ctri_reg_number, phase, therapeutic_area, status, start_date, end_date, created_by, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', [
            s.study_id,
            s.org_id,
            s.title,
            s.ctri_reg_number,
    ...
    for (const s of sites) {
        await db.run('INSERT INTO sites (site_id, study_id, name, location, lat, lng, pi_user_id) VALUES (?, ?, ?, ?, ?, ?, ?)', [s.site_id, s.study_id, s.name, s.location, s.lat, s.lng, s.pi_user_id]);
    }
    // 6. Subjects & Consent
    ...
    await db.run("INSERT INTO documents (document_id, entity_type, entity_id, file_ref, version, uploaded_by, uploaded_at) VALUES ('doc_consent_v1', 'Consent', 's_ashwagandha', '/s3/consents/ashwagandha_v1.pdf', '1.0', 'u_pi', ?)", [nowStr]);
    await db.run("INSERT INTO documents (document_id, entity_type, entity_id, file_ref, version, uploaded_by, uploaded_at) VALUES ('doc_consent_v2', 'Consent', 's_ayush64', '/s3/consents/ayush64_v1.pdf', '1.0', 'u_pi', ?)", [nowStr]);
    for (const s of subjectsData) {
        await db.run('INSERT INTO subjects (subject_id, site_id, enrollment_status, enrollment_date, dosha_profile, created_at) VALUES (?, ?, ?, ?, ?, ?)', [s.id, s.site, s.status, s.date, s.profile, nowStr]);
        ...
        await db.run('INSERT INTO subject_pii (subject_id, full_name_encrypted, contact_encrypted, abdm_health_id) VALUES (?, ?, ?, ?)', [s.id, nameEnc, contactEnc, healthId]);
        // Consent
        if (s.status !== 'Screened') {
            const consentId = `c_${s.id}`;
            const docRef = s.site.includes('goa') ? 'doc_consent_v2' : 'doc_consent_v1';
            await db.run('INSERT INTO consents (consent_id, subject_id, version, signed_date, document_ref) VALUES (?, ?, ?, ?, ?)', [consentId, s.id, '1.0', s.date, docRef]);
        }
    }
    ...
    for (const e of entries) {
        await db.run('INSERT INTO ecrf_entries (entry_id, subject_id, form_type, visit_number, entered_by, entered_at, status) VALUES (?, ?, ?, ?, ?, ?, ?)', [e.entry_id, e.subject_id, e.form_type, e.visit_number, e.entered_by, nowStr, e.status]);
    }
    // Field values
    const fields = [
        // Demographics e_001
    ...
    for (const f of fields) {
        await db.run('INSERT INTO ecrf_field_values (value_id, entry_id, field_name, field_value, sdtm_domain, sdtm_variable) VALUES (?, ?, ?, ?, ?, ?)', [f.val_id, f.entry_id, f.name, f.value, f.domain, f.variable]);
    }
    // 8. Deadline Rules (NDCT Rules 2019)
    await db.run("INSERT INTO deadline_rules (rule_id, event_type, deadline_offset_hours, escalation_thresholds_pct, escalation_chain, version, active) VALUES (?, ?, ?, ?, ?, ?, ?)", [
        'rule_sae_initial',
        'SAE',
        24,
    ...
    for (const a of aes) {
        await db.run('INSERT INTO ae_events (ae_id, subject_id, study_id, event_type, severity, causality, onset_datetime, description, reported_by, report_deadline, status, escalation_level, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', [
            a.ae_id,
            a.subject_id,
            a.study_id,
            a.event_type,
        ...
        if (a.escalation_level >= 1) {
            await db.run("INSERT INTO escalation_logs (escalation_id, ae_id, level, notified_role, notified_at, resolved) VALUES (?, ?, ?, 'IEC', ?, 0)", [`esc_${a.ae_id}_1`, a.ae_id, 1, dateHoursAgo(24 - 12)]);
        }
        if (a.escalation_level >= 2) {
            await db.run("INSERT INTO escalation_logs (escalation_id, ae_id, level, notified_role, notified_at, resolved) VALUES (?, ?, ?, 'Sponsor', ?, 0)", [`esc_${a.ae_id}_2`, a.ae_id, 2, dateHoursAgo(24 - 4)]);
        }
        if (a.escalation_level >= 3) {
            await db.run("INSERT INTO escalation_logs (escalation_id, ae_id, level, notified_role, notified_at, resolved) VALUES (?, ?, ?, 'NPvCC', ?, 0)", [`esc_${a.ae_id}_3`, a.ae_id, 3, dateHoursAgo(24 - 0)]);
        }
    }
    // 10. IEC Reviews
    const reviews = [
    ...
    for (const r of reviews) {
        await db.run('INSERT INTO iec_reviews (review_id, study_id, reviewer_id, decision, comments, decision_date, e_signature_ref) VALUES (?, ?, ?, ?, ?, ?, ?)', [r.id, r.study, r.reviewer, r.decision, r.comments, dateHoursAgo(200), r.signature]);
    }
    // 11. Milestones
    const ms = [
    ...
    for (const m of ms) {
        await db.run('INSERT INTO milestones (milestone_id, study_id, name, planned_date, actual_date, status) VALUES (?, ?, ?, ?, ?, ?)', [m.id, m.study_id, m.name, m.planned, m.actual, m.status]);
    }
    // 12. Protocol Deviations
    await db.run("INSERT INTO protocol_deviations (deviation_id, study_id, reported_by, description, severity, status, reported_at) VALUES (?, ?, ?, ?, ?, ?, ?)", [
        'dev_001',
        's_ashwagandha',
        'u_pi',
    ...
    for (const c of scorecards) {
        await db.run('INSERT INTO compliance_scores (score_id, study_id, checkpoint, status, computed_at) VALUES (?, ?, ?, ?, ?)', [c.id, c.study, c.check, c.status, nowStr]);
    }
    ...
    await db.run('INSERT INTO audit_logs (audit_id, actor_id, entity_type, entity_id, action, field, old_value, new_value, prev_hash, record_hash, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', [
        'audit_init_id',
        dummyActor,
        'Study',
        dummyEntityId,
