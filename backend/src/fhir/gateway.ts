...
export async function getFhirPatient(subjectId: string): Promise<any> {
  const db = await getDb();
  const subject = await db.get(
    `SELECT s.*, o.name as org_name, o.org_id
     FROM subjects s
...
export async function getFhirResearchStudy(studyId: string): Promise<any> {
  const db = await getDb();
...
export async function getFhirAdverseEvent(aeId: string): Promise<any> {
  const db = await getDb();
