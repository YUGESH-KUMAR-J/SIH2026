...
import { calculateDeadline } from './rules/engine';
...
async function runTests() {
  ...
  try {
    ...
    const saeDeadline = await calculateDeadline('SAE', onsetStr);
    ...
    const aeDeadline = await calculateDeadline('AE', onsetStr);
    ...
    const cleanVerification = await verifyAuditChain();
    ...
    const auditId = await writeAuditLog('u_pi', 'Subject', 'SUB-TEST-99', 'UPDATE', 'enrollment_status', 'Screened', 'Randomized');
    
    const postLogVerification = await verifyAuditChain();
    ...
    await db.run(
      "UPDATE audit_logs SET new_value = 'TAMPERED_STATUS_DATA' WHERE audit_id = ?",
      [auditId]
    );
