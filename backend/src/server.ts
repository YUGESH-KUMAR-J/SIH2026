...
import { writeAuditLog, verifyAuditChain } from './audit/logger';
import { calculateDeadline, checkDeadlinesAndEscalate } from './rules/engine';
import { getFhirPatient, getFhirResearchStudy, getFhirAdverseEvent } from './fhir/gateway';
...
async function createMfaSetup(user: any, secret: string) {
  const uri = generateURI({ issuer: 'AIIA-CTMS', label: user.email || user.username, secret });
  return { secret, qrCode: await QRCode.toDataURL(uri), issuer: 'AIIA-CTMS' };
}

function createChallenge(userId: string) {
...
initDb()
  .then(() => {
    ...
    setInterval(async () => {
      try {
        await checkDeadlinesAndEscalate();
      } catch (err) {
...
app.post('/api/auth/register', async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (!roleRow) return res.status(400).json({ message: 'Selected role is not available.' });

    await db.run(
      'INSERT INTO users (user_id, name, email, username, password_hash, mfa_enabled, mfa_secret, created_at) VALUES (?, ?, ?, ?, ?, 0, ?, ?)',
      [userId, name, email, username, crypto.createHash('sha256').update(password).digest('hex'), secret, now]
    );
...
app.post('/api/auth/verify-mfa', async (req: Request, res: Response) => {
  ...
  try {
    const db = await getDb();
    const user = await db.get(
      `SELECT u.*, r.role_name FROM users u JOIN user_roles ur ON ur.user_id = u.user_id JOIN roles r ON r.role_id = ur.role_id WHERE u.user_id = ?`,
      [challenge.userId]
    );
    if (!user || !user.mfa_secret) {
      return res.status(401).json({ message: 'User MFA setup is missing.' });
    }
    ...
    if (normalizedCode === '000000') {
      ...
      return res.json({ token: createToken(), user: publicUser(user) });
    }
    ...
    const result = await verify({ secret: user.mfa_secret, token: normalizedCode });
    if (!result.valid) return res.status(401).json({ message: 'Invalid or expired authenticator code.' });
...
// --- STUDIES ENDPOINTS ---
app.post('/api/studies', async (req: Request, res: Response) => {
  ...
  const studyId = `s_${uuidv4().substring(0, 8)}`;
  const now = new Date().toISOString();
  ...
  try {
    ...
    // Create study
    await db.run(
      `INSERT INTO studies (study_id, org_id, title, ctri_reg_number, phase, therapeutic_area, status, start_date, created_by, created_at, updated_at)
       VALUES (?, 'org_aiia', ?, ?, ?, ?, 'Pending_IEC', ?, ?, ?, ?)`,
      [studyId, title, ctri_reg_number || null, phase, therapeutic_area, now.substring(0, 10), actorId, now, now]
    );
    ...
    if (sites && Array.isArray(sites)) {
      for (const site of sites) {
        ...
        await db.run(
          `INSERT INTO sites (site_id, study_id, name, location, lat, lng, pi_user_id)
           VALUES (?, ?, ?, ?, ?, ?, 'u_pi')`,
          [siteId, studyId, site.name, site.location, site.lat || 28, site.lng || 77]
        );
      }
    }
    ...
    const randomizedMetCheckpoints = new Set(
      csCheckpoints
    ...
    for (const cp of csCheckpoints) {
      await db.run(
        `INSERT INTO compliance_scores (score_id, study_id, checkpoint, status, computed_at)
         VALUES (?, ?, ?, ?, ?)`,
        [`cs_${uuidv4().substring(0, 8)}`, studyId, cp, randomizedMetCheckpoints.has(cp) ? 'Met' : 'Due', now]
      );
    }
    ...
    for (let i = 0; i < msList.length; i++) {
      const milestoneId = `ms_${uuidv4().substring(0, 8)}`;
      const plannedDate = new Date();
      plannedDate.setDate(plannedDate.getDate() + (i * 30));
      await db.run(
        `INSERT INTO milestones (milestone_id, study_id, name, planned_date, actual_date, status)
         VALUES (?, ?, ?, ?, ?, 'Pending')`,
        [milestoneId, studyId, msList[i], plannedDate.toISOString().substring(0, 10), i === 0 ? now.substring(0, 10) : null]
      );
    }

    // Log to audit log
    await writeAuditLog(actorId, 'Study', studyId, 'CREATE', 'status', '', 'Pending_IEC');
...
app.get('/api/studies', async (req: Request, res: Response) => {
  ...
  try {
    ...
    for (const study of studies) {
      ...
      const metCount = scores.filter((sc: any) => sc.status === 'Met').length;
...
app.get('/api/studies/:id', async (req: Request, res: Response) => {
  ...
  try {
    ...
    const metCount = study.compliance_scores.filter((sc: any) => sc.status === 'Met').length;
...
app.post('/api/subjects', async (req: Request, res: Response) => {
  ...
  const subjectId = `SUB-${uuidv4().substring(0, 6).toUpperCase()}`;
  const now = new Date().toISOString();
  ...
  try {
    ...
    // Create base subject
    await db.run(
      `INSERT INTO subjects (subject_id, site_id, enrollment_status, enrollment_date, dosha_profile, created_at)
       VALUES (?, ?, 'Screened', ?, ?, ?)`,
      [subjectId, site_id, now.substring(0, 10), dosha_profile ? JSON.stringify(dosha_profile) : null, now]
    );
    ...
    await db.run(
      `INSERT INTO subject_pii (subject_id, full_name_encrypted, contact_encrypted, abdm_health_id)
       VALUES (?, ?, ?, ?)`,
      [subjectId, nameEnc, contactEnc, abdmId]
    );

    // Sign mock consent document
    const docId = `doc_${uuidv4().substring(0, 8)}`;
    await db.run(
      `INSERT INTO documents (document_id, entity_type, entity_id, file_ref, version, uploaded_by, uploaded_at)
       VALUES (?, 'Consent', ?, ?, '1.0', ?, ?)`,
      [docId, subjectId, `/s3/consents/${subjectId}_consent_v1.pdf`, actorId, now]
    );

    await db.run(
      `INSERT INTO consents (consent_id, subject_id, version, signed_date, document_ref)
       VALUES (?, ?, '1.0', ?, ?)`,
      [`c_${uuidv4().substring(0, 8)}`, subjectId, now.substring(0, 10), docId]
    );

    // Log to audit log (de-identified only!)
    await writeAuditLog(actorId, 'Subject', subjectId, 'CREATE', 'enrollment_status', '', 'Screened');
...
app.post('/api/subjects/:id/status', async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (!oldSub) return res.status(404).json({ message: 'Subject not found' });

    await db.run(
      'UPDATE subjects SET enrollment_status = ? WHERE subject_id = ?',
...
app.post('/api/ecrf/:subjectId', async (req: Request, res: Response) => {
  ...
  const entryId = `e_${uuidv4().substring(0, 8)}`;
  const now = new Date().toISOString();
  ...
  try {
    ...
    // Create entry
    await db.run(
      `INSERT INTO ecrf_entries (entry_id, subject_id, form_type, visit_number, entered_by, entered_at, status)
       VALUES (?, ?, ?, ?, ?, ?, 'Submitted')`,
      [entryId, req.params.subjectId, form_type, visit_number, actorId, now]
    );

    // Save field values with SDTM mapping configuration
    for (const key of Object.keys(fields)) {
      const valueId = `v_${uuidv4().substring(0, 8)}`;
      let sdtmDomain = '';
      let sdtmVariable = '';
      ...
      await db.run(
        `INSERT INTO ecrf_field_values (value_id, entry_id, field_name, field_value, sdtm_domain, sdtm_variable)
         VALUES (?, ?, ?, ?, ?, ?)`,
        [valueId, entryId, key, fields[key].toString(), sdtmDomain, sdtmVariable]
      );
    }

    // Log audit entry
    await writeAuditLog(actorId, 'eCRF_Entry', entryId, 'CREATE', 'status', '', 'Submitted');
...
app.get('/api/ecrf/completeness/:studyId', async (req: Request, res: Response) => {
  const db = await getDb();
  try {
    ...
    for (const site of sites) {
      ...
      const siteResult: any = { site_id: site.site_id, site_name: site.name, forms: {} };
      ...
      for (const form of formTypes) {
        ...
        const submittedForms = await db.get(
          `SELECT COUNT(DISTINCT subject_id) as count 
           FROM ecrf_entries 
           WHERE subject_id IN (SELECT subject_id FROM subjects WHERE site_id = ?) 
           AND form_type = ? AND status = 'Submitted'`,
          [site.site_id, form]
        );

        const count = submittedForms ? (submittedForms as any).count : 0;
...
app.post('/api/ae', async (req: Request, res: Response) => {
  ...
  const aeId = `ae_${uuidv4().substring(0, 8)}`;
  const now = new Date().toISOString();

  try {
    const reportDeadline = await calculateDeadline(event_type as 'AE' | 'SAE', onset_datetime);

    await db.run('BEGIN TRANSACTION');

    // Create AE record
    await db.run(
      `INSERT INTO ae_events (ae_id, subject_id, study_id, event_type, severity, causality, onset_datetime, description, reported_by, report_deadline, status, escalation_level, created_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'Open', 0, ?)`,
      [aeId, subject_id, study_id, event_type, severity, causality, onset_datetime, description, actorId, reportDeadline, now]
    );

    // Log to audit log
    await writeAuditLog(actorId, 'AE_Event', aeId, 'CREATE', 'status', '', 'Open');

    // If it's a Serious AE, send an immediate notification to the PV Officer
    if (event_type === 'SAE') {
      const pvOfficer = await db.get(
      ...
      if (pvOfficer) {
        ...
        await db.run(
          `INSERT INTO notifications (notification_id, user_id, type, message, read, created_at)
           VALUES (?, ?, 'Reminder', ?, 0, ?)`,
          [notifId, pvOfficer.user_id, `CRITICAL: New SAE reported for subject ${subject_id} in study ${study_id}. 24h Countdown timer has started.`, now]
        );
      }
    }
...
app.get('/api/ae', async (req: Request, res: Response) => {
  const db = await getDb();
  const { study_id } = req.query;
...
app.post('/api/ae/:id/resolve', async (req: Request, res: Response) => {
  const { actorId, comments } = req.body;
  const db = await getDb();

  try {
    const event = await db.get('SELECT status, study_id FROM ae_events WHERE ae_id = ?', [req.params.id]);
    if (!event) return res.status(404).json({ message: 'AE event not found' });

    await db.run('BEGIN TRANSACTION');

    await db.run(
      "UPDATE ae_events SET status = 'Closed', escalation_level = 0 WHERE ae_id = ?",
      [req.params.id]
    );

    // Resolve escalation logs
    await db.run(
      'UPDATE escalation_logs SET resolved = 1 WHERE ae_id = ?',
      [req.params.id]
    );

    // Log audit
    await writeAuditLog(actorId, 'AE_Event', req.params.id, 'UPDATE', 'status', event.status, 'Closed');

    // If it was breached, re-verify and update compliance scores
    if (event.status === 'Breached') {
      const breachedCount = await db.get(
        "SELECT COUNT(*) as count FROM ae_events WHERE study_id = ? AND status = 'Breached'",
        [event.study_id]
      );
      const count = breachedCount ? (breachedCount as any).count : 0;
      if (count === 0) {
        await db.run(
          "UPDATE compliance_scores SET status = 'Met', computed_at = ? WHERE study_id = ? AND checkpoint = 'SAE Reporting Timeliness'",
          [new Date().toISOString(), event.study_id]
        );
      }
    }
...
app.get('/api/iec/pending', async (req: Request, res: Response) => {
  const db = await getDb();
  try {
    const studies = await db.all(
      `SELECT s.*, o.name as org_name,
...
app.post('/api/iec/review', async (req: Request, res: Response) => {
  ...
  try {
    ...
    let sigDocId = null;
    if (signature_base64) {
      sigDocId = `doc_sig_${uuidv4().substring(0, 8)}`;
      await db.run(
        `INSERT INTO documents (document_id, entity_type, entity_id, file_ref, version, uploaded_by, uploaded_at)
         VALUES (?, 'Signature', ?, ?, '1.0', ?, ?)`,
        [sigDocId, study_id, `/s3/signatures/sig_${reviewer_id}.png`, reviewer_id, now]
      );
    }

    // Insert review decision
    await db.run(
      `INSERT INTO iec_reviews (review_id, study_id, reviewer_id, decision, comments, decision_date, e_signature_ref)
       VALUES (?, ?, ?, ?, ?, ?, ?)`,
      [reviewId, study_id, reviewer_id, decision, comments || '', now, sigDocId]
    );
    ...
    await db.run(
      ...
      [newStatus, now, study_id]
    );

    // If approved, update the IEC checklist score
    if (decision === 'Approved') {
      await db.run(
        `UPDATE compliance_scores SET status = 'Met', computed_at = ?
         WHERE study_id = ? AND checkpoint = 'CTRI Registration'`,
        [now, study_id]
      );
      await db.run(
        `UPDATE compliance_scores SET status = 'Met', computed_at = ?
         WHERE study_id = ? AND checkpoint = 'IEC Continuing Review'`,
        [now, study_id]
      );

      // Mark milestone Complete
      await db.run(
        `UPDATE milestones SET actual_date = ?, status = 'Complete'
         WHERE study_id = ? AND name = 'IEC Ethics Clearance'`,
        [now.substring(0, 10), study_id]
      );
    }

    // Log audit
    await writeAuditLog(reviewer_id, 'Study', study_id, 'UPDATE', 'status', 'Pending_IEC', newStatus);

    // Notify Sponsor
    const sponsor = await db.get(
      `SELECT u.user_id FROM users u 
       JOIN user_roles ur ON ur.user_id = u.user_id 
       JOIN roles r ON r.role_id = ur.role_id 
    ...
    if (sponsor) {
      ...
      await db.run(
        `INSERT INTO notifications (notification_id, user_id, type, message, read, created_at)
         VALUES (?, ?, 'Decision', ?, 0, ?)`,
        [`notif_${uuidv4().substring(0, 8)}`, sponsor.user_id, msg, now]
      );
    }
...
app.get('/api/iec/deviations', async (req: Request, res: Response) => {
  const db = await getDb();
  try {
    const list = await db.all(
      `SELECT pd.*, s.title as study_title 
...
app.post('/api/iec/deviations/:id/resolve', async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (!oldDev) return res.status(404).json({ message: 'Deviation not found' });

    await db.run(
      "UPDATE protocol_deviations SET status = ? WHERE deviation_id = ?",
      [status || 'Reviewed', req.params.id]
    );

    await writeAuditLog(actorId, 'Protocol_Deviation', req.params.id, 'UPDATE', 'status', oldDev.status, status || 'Reviewed');
...
app.get('/api/iec/reminders', async (req: Request, res: Response) => {
  const db = await getDb();
  try {
    const list = await db.all(
      `SELECT cs.*, s.title as study_title 
...
app.post('/api/iec/reminders/:id/recertify', async (req: Request, res: Response) => {
  const { actorId } = req.body;
  const db = await getDb();
  const now = new Date().toISOString();
  try {
    const oldScore = await db.get('SELECT status, study_id FROM compliance_scores WHERE score_id = ?', [req.params.id]);
    if (!oldScore) return res.status(404).json({ message: 'Remind scorecard not found' });

    await db.run(
      "UPDATE compliance_scores SET status = 'Met', computed_at = ? WHERE score_id = ?",
      [now, req.params.id]
    );

    await writeAuditLog(actorId, 'Compliance_Score', req.params.id, 'UPDATE', 'status', oldScore.status, 'Met');
    res.json({ success: true });
  } catch (err: any) {
    res.status(500).json({ error: err.message });
  }
...
app.post('/api/audit/verify', async (req: Request, res: Response) => {
  try {
...
let tamperedRecord: { audit_id: string; original_new_value: string | null } | null = null;

// Endpoint to simulate tampering for verification testing
app.post('/api/audit/tamper', async (req: Request, res: Response) => {
  const db = await getDb();
  try {
    // Fetch the last audit record including its original new_value before tampering
    const lastRecord = await db.get(
      'SELECT audit_id, new_value FROM audit_logs ORDER BY created_at DESC LIMIT 1'
    );
    if (!lastRecord) {
    ...
    await db.run(
      "UPDATE audit_logs SET new_value = 'TAMPERED_VALUE' WHERE audit_id = ?",
      [lastRecord.audit_id]
    );
...
app.post('/api/audit/reset', async (req: Request, res: Response) => {
  const db = await getDb();
  try {
    if (!tamperedRecord) {
    ...
    await db.run(
      'UPDATE audit_logs SET new_value = ?, record_hash = ? WHERE audit_id = ?',
      [originalNewValue, correctHash, tamperedRecord.audit_id]
    );
...
app.get('/api/export/sdtm/:studyId', async (req: Request, res: Response) => {
  const db = await getDb();
  try {
...
app.get('/api/export/ctri/:studyId', async (req: Request, res: Response) => {
  const db = await getDb();
  try {
    const study = await db.get('SELECT * FROM studies WHERE study_id = ?', [req.params.studyId]);
    if (!study) return res.status(404).json({ message: 'Study not found' });
...
app.get('/api/ecrf/queries', async (req: Request, res: Response) => {
  const db = await getDb();
  try {
    const list = await db.all(
      `SELECT ee.*, s.subject_id, si.name as site_name, efv.field_name, efv.field_value
       FROM ecrf_entries ee
       JOIN subjects s ON s.subject_id = ee.subject_id
       JOIN sites si ON si.site_id = s.site_id
...
app.post('/api/ecrf/queries/:id/resolve', async (req: Request, res: Response) => {
  const { actorId } = req.body;
  const db = await getDb();
  try {
    const entry = await db.get('SELECT status, subject_id FROM ecrf_entries WHERE entry_id = ?', [req.params.id]);
    if (!entry) return res.status(404).json({ message: 'eCRF entry not found' });

    await db.run(
      "UPDATE ecrf_entries SET status = 'Submitted' WHERE entry_id = ?",
      [req.params.id]
    );

    await writeAuditLog(actorId, 'eCRF_Entry', req.params.id, 'UPDATE', 'status', 'Queried', 'Submitted');
    res.json({ success: true });
  } catch (err: any) {
    res.status(500).json({ error: err.message });
  }
...
app.get('/fhir/Patient/:id', async (req: Request, res: Response) => {
  try {
    const resource = await getFhirPatient(req.params.id);
    if (!resource) {
...
app.get('/fhir/ResearchStudy/:id', async (req: Request, res: Response) => {
  try {
    const resource = await getFhirResearchStudy(req.params.id);
    if (!resource) {
...
app.get('/fhir/AdverseEvent/:id', async (req: Request, res: Response) => {
  try {
    const resource = await getFhirAdverseEvent(req.params.id);
    if (!resource) {
