 ...
 */
export async function calculateDeadline(eventType: 'AE' | 'SAE', onsetDatetimeStr: string): Promise<string> {
  const db = await getDb();
  const rule = await db.get(
    'SELECT deadline_offset_hours FROM deadline_rules WHERE event_type = ? AND active = 1 LIMIT 1',
    [eventType]
  );

  const offsetHours = rule ? (rule as any).deadline_offset_hours : 24; // Default 24h for SAE, 168h for AE
 ...
 */
export async function checkDeadlinesAndEscalate(): Promise<void> {
  const db = await getDb();

  // Find all open events (AE or SAE)
  const openEvents = await db.all(
    `SELECT ae.*, dr.deadline_offset_hours, dr.escalation_thresholds_pct, dr.escalation_chain
     FROM ae_events ae
  ...
  for (const event of openEvents) {
    ...
    // Parse rules
    const thresholds = JSON.parse(event.escalation_thresholds_pct) as number[]; // e.g. [50, 80, 100]
    const chain = JSON.parse(event.escalation_chain) as string[];             // e.g. ["PI", "IEC", "Sponsor", "NPvCC"]
    ...
    if (targetLevel > event.escalation_level) {
      ...
      try {
        // Update ae_events
        await db.run(
          'UPDATE ae_events SET escalation_level = ?, status = ? WHERE ae_id = ?',
          [targetLevel, newStatus, event.ae_id]
        );

        // Audit log the update
        await writeAuditLog(
          'u_pv', // System / PV Actor ID representing background escalation
          'AE_Event',
          event.ae_id,
          'UPDATE',
          'escalation_level',
          event.escalation_level.toString(),
        ...
        if (newStatus === 'Breached') {
          ...
          await db.run(
            `UPDATE compliance_scores
             ...
             WHERE study_id = ? AND checkpoint = 'SAE Reporting Timeliness'`,
            [now.toISOString(), event.study_id]
          );
        }
        ...
        await db.run(
          'INSERT INTO escalation_logs (escalation_id, ae_id, level, notified_role, notified_at, resolved) VALUES (?, ?, ?, ?, ?, 0)',
          [escalationId, event.ae_id, targetLevel, notifiedRole, now.toISOString()]
        );
        ...
        for (const user of usersToNotify) {
          ...
          await db.run(
            'INSERT INTO notifications (notification_id, user_id, type, message, read, created_at) VALUES (?, ?, ?, ?, 0, ?)',
            [notificationId, user.user_id, 'Escalation', message, now.toISOString()]
          );
        }
